...
def _split_by_size(
    ...
    for para in paragraphs:
        # Nếu thêm paragraph này vào mà vẫn chưa quá giới hạn
        if len(current_chunk_text) + len(para) <= chunk_chars:
...
def get_embedding(text: str) -> List[float]:
    ...
    if not api_key:
        raise ValueError("JINA_API_KEY không được tìm thấy trong .env")
    ...
    if response.status_code != 200:
        raise Exception(f"Jina Embedding Error: {response.text}")
...
def build_index(docs_dir: Path = DOCS_DIR, db_dir: Path = CHROMA_DB_DIR) -> None:
    ...
    print(f"  [ChromaDB] Khởi tạo database tại: {db_dir}")
    ...
    for filepath in doc_files:
        ...
        # Step 1: Preprocess
        print(f"  [1/3] Preprocessing: Extracting metadata and cleaning text...")
        doc = preprocess_document(raw_text, str(filepath))
        print(f"    Metadata found: {doc['metadata']}")
        
        # Step 2: Chunk
        print(f"  [2/3] Chunking: Splitting document into smaller sections...")
        chunks = chunk_document(doc)
        print(f"    -> Generated {len(chunks)} chunks")

        # Step 3: Embed and Store
        print(f"  [3/3] Vectorizing: Embedding and uploading to ChromaDB...")
        ...
        if ids:
            ...
            print(f"    -> Successfully uploaded {len(ids)} chunks to ChromaDB")
        
        total_chunks += len(chunks)

    print(f"\n{'='*20}")
    print(f"Hoàn thành Build Index!")
    ...
    try:
        ...
        if results["documents"]:
            for i, (doc, meta) in enumerate(zip(results["documents"], results["metadatas"])):
                ...
                print(f"  effective_date: {meta.get('effective_date', 'N/A')},")
